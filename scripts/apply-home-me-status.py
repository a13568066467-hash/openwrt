...
def upload(client: paramiko.SSHClient, local: Path, remote: str, mode: int = 0o755):
...
grep -n hostname_for_mac /usr/lib/nds-agent/cloud.uc | head -3
...
"""
    stdin, stdout, stderr = c.exec_command(setup, timeout=90)
    stdin.channel.shutdown_write()
